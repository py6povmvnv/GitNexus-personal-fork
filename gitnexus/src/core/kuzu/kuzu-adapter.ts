...
export const loadExistingGraph = async (
  ...
  try {
    ...
    for (const row of await result.getAll()) {
      ...
      // Only keep DEFINES (intra-file, deterministic IDs, only re-created for changed files).
      // Everything else is regenerated:
      // - CONTAINS: by processStructure on all files
      // - IMPORTS, CALLS, EXTENDS, IMPLEMENTS: by full import/call/heritage on all files
      // - MEMBER_OF, STEP_IN_PROCESS: by community/process detection
